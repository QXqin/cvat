...
| 快捷键 | 功能描述 | 技术实现 |
| :--- | :--- | :--- |
| `D / F` | 上一帧 / 下一帧 | 调用底层 Frame Controller API |
| `C / V` | 快进/快退 (±10) | 步进式渲染优化 |
| `Space` | 状态切换 | 拦截全局 Redux 快捷键，防止主屏误触 |
...
> **⚠️ 版本兼容性提醒：**
> 本扩展插件基于 **CVAT v2.56.2** 版本体系开发。因涉及底层标注数据的序列化与反序列化，强烈建议在同等或兼容版本下应用此扩展。
